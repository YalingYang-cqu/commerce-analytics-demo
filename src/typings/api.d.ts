...
declare namespace Api {
  ...
  namespace Auth {
    ...
    /** 用户信息（store 中按 Partial 使用；登录接口可能多返回 account、phoneNum 等） */
    interface UserInfo {
      buttons: string[]
      roles: string[]
      userId: number
  ...
  namespace LogisticsMethod {
    ...
    interface ListItem {
      ...
      /** 目的国多选：每项为 "洲" 或 "洲/国家" */
      destinationCountries?: string[]
      /** 支持收货仓类型（API：FBA_ADDRESS、OVERSEAS_WAREHOUSE） */
      receiveWarehouseTypes?: string[]
      /** 分区规则（API：BY_COUNTRY、BY_POSTCODE、BY_FBA_WAREHOUSE） */
      zoneRule?: string
      /** 按邮编分区时的邮编匹配表达式（仅 BY_POSTCODE） */
      postcodeRange?: string
      /** 按 FBA 仓库分区时的仓库代码列表（仅 BY_FBA_WAREHOUSE） */
      fbaWarehouseCodes?: string[]
      /** 可接货物属性 */
    ...
    interface SaveParams {
      ...
      /** 按 FBA 仓库分区时的仓库代码列表 */
      fbaWarehouseCodes?: string[]
      /** 可接货物属性（多选为 string[]，兼容单值 string） */
      cargoAttribute?: CargoAttribute | string | string[]
      /** 是否包税 */
      includeTax: boolean
      /** 是否接一般贸易 */
      acceptGeneralTrade?: boolean
  ...
  namespace Waybill {
    ...
    interface Detail {
      ...
      shipToStreet?: string
      /** 目的地-邮编 */
      shipToPostCode?: string | null | undefined
      // 汇总信息
      /** 总箱数 */
      boxCount?: number
      /** 商品数量 */
    ...
    /** 新增附件返回（TmsWaybillAttachment 对象） */
    interface AddAttachmentVo {
      /** 主键ID */
      id: number
      /** 业务ID（运单ID/调拨单ID） */
      businessId?: number
