...
/**
 * html2canvas 已知 bug：给 input 设置 height/font-size 后，截图时不会按输入框的
 * 垂直居中绘制文字，导致文字偏上。可靠修复是在克隆阶段把文本 input 替换成等价的
 * span，用 flex 做垂直居中（仅处理文本输入框，复选框/单选框不动）
 */
function centerTextInputsInClone(clonedDoc: Document): void {
  const view = clonedDoc.defaultView
  // 只匹配真正的 <input>；周选择器里的 .el-range-input 是 <span>，不能被当成 input 取值
  clonedDoc
    .querySelectorAll<HTMLInputElement>('input.el-range-input, input.el-input__inner')
    .forEach((input) => {
      ...
      span.style.justifyContent = cs?.textAlign === 'center' ? 'center' : 'flex-start'
  ...
  try {
    ...
    const canvas = await html2canvas(element, {
      ...
      onclone: (clonedDoc) => {
        ...
        // 修复 html2canvas 对 input 文字垂直居中的渲染 bug（日期框文字偏上）
        centerTextInputsInClone(clonedDoc)
      }
    })
