  ...
  <div class="cost-trend-detail-donut">
    ...
    <ul class="donut-legend">
      <li v-for="item in items" :key="item.name">
        <span class="dot" :style="{ background: getChannelBarColor(item.name) }" />
        <span class="name">{{ formatChannelShortName(item.name) }}</span>
        <span class="amount">¥{{ formatMoney(item.amount) }}</span>
        <span class="ratio">{{ item.ratio }}%</span>
      </li>
    </ul>
    <p class="donut-tip">点击物流渠道可在主图筛选 →</p>
  </div>
</template>
...
<script setup lang="ts">
  ...
  import { formatChannelShortName, getChannelBarColor } from '../utils/channel-tag'
  ...
  watch(() => [props.items, props.centerAmount] as const, refreshChart, { deep: true })
