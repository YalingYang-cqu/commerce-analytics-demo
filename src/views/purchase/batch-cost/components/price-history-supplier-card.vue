<!-- 单供应商历史价格卡片 -->
<template>
  <ElCard shadow="never" class="supplier-price-card" :style="accentVars">
    <div class="supplier-price-card__head">
      <div class="supplier-price-card__head-left">
        <span
          :class="getSupplierTagClass(supplierIndex, supplier.supplierName)"
          :title="supplier.supplierName"
        >
          {{ formatSupplierShortName(supplier.supplierName) }}
        </span>
    ...
    <div class="supplier-price-card__body">
      <div class="version-tables" :class="{ 'is-split': isSplit }">
        ...
        >
          ...
          <template #price="{ row }">
            <!-- <span class="price-estimate-tag" v-if="row.costType === 'estimate'">(预估)</span> -->
            <span class="col-price">¥{{ formatMoney(row.price) }}</span>
          </template>
          <template #change="{ row }">
            <span
...
<script setup lang="ts">
  ...
  import type { SupplierPriceHistory } from '../utils/mock-price-history'
  import { formatSupplierShortName, getSupplierTagClass } from '../utils/supplier-tag'
  ...
  const versionColumns = [
    { prop: 'version', label: '版本', minWidth: 70, useSlot: true },
    { prop: 'price', label: '价格', minWidth: 100, useSlot: true },
    { prop: 'change', label: '变化', minWidth: 80, useSlot: true },
    { prop: 'effectiveDate', label: '生效日期', minWidth: 110, useSlot: true }
  ]
