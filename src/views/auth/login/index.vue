...
<script setup lang="ts">
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const loginRes: any = await fetchLogin({
        userName: username,
        password
      })
      const tk: string | undefined = loginRes?.token ?? loginRes?.data?.token
      const rt: string | undefined = loginRes?.refreshToken ?? loginRes?.data?.refreshToken

      if (!tk) {
        throw new Error('Login failed - no token received')
      }

      userStore.setToken(`${tk}`, rt)
