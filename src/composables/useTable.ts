...
> {
  ...
  core: {
    ...
    tableKey?: string
    /** 列配置持久化标识，与 ArtTableHeader tableName 一致，用于恢复列配置 */
    tableName?: string
  }
...
function useTableImpl<TApiFn extends (params: any) => Promise<any>>(
  ...
  const pageKey = paginationKey?.currentPage || tableConfig.paginationKey.current
  const sizeKey = paginationKey?.pageSize || tableConfig.paginationKey.size
  ...
  // 获取路由信息（用于 tableKey 未提供时）
  const route = useRoute()

  // 获取实际的 storage key
  const getStorageKey = (): string => {
    const key = tableKey || route.path
    return `art-table-pageSize-${key}`
  }

  // 从 localStorage 读取缓存的 pageSize
  const getCachedPageSize = (): number | null => {
    try {
      const storageKey = getStorageKey()
      const cached = localStorage.getItem(storageKey)
      if (cached) {
        const pageSize = Number.parseInt(cached, 10)
        // 验证是否为有效数字且大于 0
  ...
  const savePageSizeToStorage = (pageSize: number): void => {
    try {
      ...
      if (enableLog) {
        logger.warn('保存 pageSize 到 localStorage 失败:', error)
      }
    }
  }
  ...
  // 获取初始 pageSize：优先使用缓存，其次使用 apiParams，最后使用默认值 20
  const getInitialPageSize = (): number => {
    const cachedPageSize = getCachedPageSize()
    if (cachedPageSize !== null) {
      return cachedPageSize
    }
    const paramsRecord = apiParams as Record<string, unknown>
    return (paramsRecord[sizeKey] as number) || 20
  }

  const initialPageSize = getInitialPageSize()

  // 搜索参数：优先使用缓存的 pageSize，确保缓存值不会被 apiParams 覆盖
  const baseParams = {
    [pageKey]: 1,
    [sizeKey]: initialPageSize
  }
  const mergedParams = Object.assign({}, baseParams, apiParams || {}) as Record<string, unknown>
  // 确保使用初始 pageSize（已优先使用缓存值），防止被 apiParams 覆盖
  mergedParams[sizeKey] = initialPageSize
  const searchParams = reactive(mergedParams as TParams)

  // 分页配置
  const pagination = reactive<Api.Common.PaginationParams>({
    currentPage: ((searchParams as Record<string, unknown>)[pageKey] as number) || 1,
  ...
  const columnConfig = columnsFactory
    ? useTableColumns<TRecord>(columnsFactory, tableName)
    : null
    ...
    try {
      ...
      if (useCache && cache) {
        ...
        if (cachedItem) {
          ...
          if (paramsRecord[pageKey] !== pagination.currentPage) {
            paramsRecord[pageKey] = pagination.currentPage
          }
          if (paramsRecord[sizeKey] !== pagination.pageSize) {
            paramsRecord[sizeKey] = pagination.pageSize
          }
  ...
  const resetSearchParams = async (): Promise<void> => {
    ...
    // 重置分页
    pagination.currentPage = 1
    pagination.pageSize = defaultPagination[sizeKey] as number
  ...
  const handleCurrentChange = async (newCurrent: number): Promise<void> => {
    ...
    if (pagination.currentPage === newCurrent) {
    ...
    try {
      ...
      // 修复：只更新必要的状态
      const paramsRecord = searchParams as Record<string, unknown>
      pagination.currentPage = newCurrent
  ...
  const refreshRemove = async (): Promise<void> => {
    ...
    // 如果当前页为空且不是第一页，回到上一页
    if (data.value.length === 0 && currentPage > 1) {
      pagination.currentPage = currentPage - 1
      ;(searchParams as Record<string, unknown>)[pageKey] = currentPage - 1
      await getData()
    }
  }
