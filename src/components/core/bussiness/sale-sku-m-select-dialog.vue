...
<script setup lang="ts">
  ...
  import { pubStoreList, pubGetMainList } from '@/utils'
  ...
  const searchForm = reactive({
    searchType: 'sku',
    searchValue: '',
    companySubjectIdList: [] as (string | number)[],
    storeIdList: [] as (string | number)[]
  })

  const storeList = ref<Array<{ label: string; value: string | number }>>([])
  const companyList = ref<Array<{ label: string; value: string | number }>>([])
  ...
  function transformRecord(v: any) {
    ...
    return {
      ...
      companySubjectName: v.companySubjectName ?? v.companySubject ?? '',
  ...
  const columnsFactory = () => [
    ...
    { prop: 'companySubjectName', label: '公司主体', minWidth: 140 },
  ...
  const {
    ...
    core: {
      apiFn: async (params: any) => {
        const apiParams: any = {
          ...
          companySubjectIdList: params.companySubjectIdList,
    ...
    hooks: {
      resetFormCallback: () => {
        ...
        searchForm.companySubjectIdList = []
  ...
  async function loadOptions() {
    try {
      const [companyRes, storeRes]: any[] = await Promise.all([
        pubGetMainList().catch(() => []),
        pubStoreList({ status: null }).catch(() => [])
      ])
      const companyData = companyRes?.data ?? companyRes ?? []
      const storeData = storeRes?.data ?? storeRes ?? []
      companyList.value = Array.isArray(companyData)
        ? companyData.map((c: any) => ({
            label: c.companySubjectName ?? c.label ?? c.name ?? '',
            value: c.id ?? c.value
          }))
        : []
      storeList.value = Array.isArray(storeData)
        ? storeData.map((s: any) => ({
