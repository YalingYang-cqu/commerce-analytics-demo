...
<template>
  <div
    :class="{
      'is-empty': isEmpty,
      'art-table': true,
      'art-table-group': true,
      'has-group-bar': groupBarEnabled,
      'no-border': !border,
      'summary-top': props.summaryPosition === 'top'
    }"
    :style="containerHeight"
    ref="containerRef"
  >
    <!-- 彩色分组条：位于 ElTable 上方（表格外），按分组合并、宽度对齐列宽并随表横向滚动 -->
    <div v-if="groupBarEnabled && groupBarSegments.length" class="atg-group-bar">
      <div class="atg-group-bar__track" :style="{ transform: `translateX(${-scrollLeft}px)` }">
        <div
          v-for="(seg, i) in groupBarSegments"
          :key="i"
          class="atg-group-seg"
          :class="{ 'is-group': seg.isGroup }"
          :style="{ width: seg.width + 'px' }"
        >
          <template v-if="seg.isGroup">
            <span class="atg-group-pill" :style="{ backgroundColor: seg.color }">{{
              seg.title
            }}</span>
            <span class="atg-group-line" :style="{ backgroundColor: seg.color }"></span>
          </template>
        </div>
      </div>
    </div>
            ...
            <ElTableColumn v-bind="cleanColumnProps(childCol)">
              ...
              <template v-else-if="childCol.ellipsis && childCol.prop" #default="slotScope">
                <ElTooltip
                  ...
                  placement="auto"
                  popper-class="art-table-ellipsis-tooltip"
                  :teleported="true"
                  :popper-options="ellipsisTooltipPopperOptions"
                >
...
<script setup lang="ts">
  ...
  defineOptions({ name: 'ArtTableGroup' })

  const { width, height: windowHeight } = useWindowSize()
  const route = useRoute()
  const ellipsisTooltipPopperOptions = {
  ...
  interface ArtTableProps extends TableProps<Record<string, any>> {
    ...
    /** 翻页后是否将整个页面滚动到顶部；默认开启 */
    scrollPageToTopOnPageChange?: boolean
    /** 空数据表格高度 */
    emptyHeight?: string
    /** 空数据时显示的文本 */
    emptyText?: string
    ...
    tableKey?: string
    tableName?: string
    /** 是否开启列宽记忆（拖拽列宽保存到接口，刷新后恢复），依赖 tableKey 或路由 path 作为习惯 key */
    habitColumnWidth?: boolean
    autoHeight?: boolean
    /** 分组定义：列通过 group 字段归属到某个分组，表头会自动渲染分组色条与首列分组标签 */
    groups?: GroupOption[]
    /** 合计行位置，默认 bottom；为 top 时合计行固定在表头下方顶部（需配合 show-summary 使用） */
    summaryPosition?: 'top' | 'bottom'
  }

  /** 分组定义 */
  interface GroupOption {
    /** 分组唯一标识，与列的 group 字段对应 */
    key: string
    /** 分组标题（彩色标签文案） */
    title: string
    /** 分组主题色 */
  ...
  const props = withDefaults(defineProps<ArtTableProps>(), {
    ...
    scrollPageToTopOnPageChange: true
  })
  ...
  const computedTableHeight = computed(() => {
    ...
    if (containerRef.value && windowHeight.value > 0) {
      ...
      // 额外的安全边距（避免紧贴边缘）
      const safeMargin = 20
      // 分组条占用高度（含底部色线），避免表格超出视口
      const groupBarOffset = groupBarEnabled.value ? GROUP_BAR_HEIGHT : 0
  ...
  // 列宽习惯：用于接口记忆列宽，key 与 tableKey/route 一致
  ...
  // 有 columnChecks 时按 columnChecks 顺序与显隐得到列，保证拖拽列顺序实时生效；key 与 useTableColumns 的 getColumnKey 一致（selection 为 __selection__）以保证勾选列能匹配
  ...
  // ========== 彩色分组条（ElTable 上方覆盖条：按分组合并 + 量列宽对齐 + 随表横向滚动） ==========
  /** 是否启用分组 */
  const groupBarEnabled = computed(() => (props.groups?.length ?? 0) > 0)
  /** 分组定义索引：key -> group */
  const groupMetaMap = computed(() => {
    const map = new Map<string, GroupOption>()
    ;(props.groups || []).forEach((g) => map.set(g.key, g))
    return map
  })

  /** 各可见列实测宽度（按 displayColumns 顺序） */
  const measuredWidths = ref<number[]>([])
  /** 表体横向滚动偏移，用于让分组条与表体同步 */
  const scrollLeft = ref(0)
  let scrollEl: HTMLElement | null = null
  let groupResizeObserver: ResizeObserver | null = null

  /** 当前组件直属的 ElTable；展开行中的嵌套表格不能参与主表分组条计算 */
  const getOwnTableRoot = () =>
    containerRef.value?.querySelector<HTMLElement>(':scope > .el-table') ?? null

  const onBodyScroll = () => {
    if (scrollEl) scrollLeft.value = scrollEl.scrollLeft
  }

  /** 读取表头各列实际宽度（排除右侧 gutter 占位列） */
  const measureColumns = () => {
    if (!groupBarEnabled.value) return
    const tableRoot = getOwnTableRoot()
    if (!tableRoot) return
    const ths = tableRoot.querySelectorAll<HTMLElement>(
      ':scope > .el-table__inner-wrapper > .el-table__header-wrapper thead tr th.el-table__cell:not(.gutter)'
    )
    if (!ths.length) return
    measuredWidths.value = Array.from(ths).map((th) => th.getBoundingClientRect().width)
  }

  /** 绑定表体横向滚动监听 */
  const bindBodyScroll = () => {
    const next = getOwnTableRoot()?.querySelector<HTMLElement>(
      ':scope > .el-table__inner-wrapper > .el-table__body-wrapper .el-scrollbar__wrap'
    )
  ...
  /** 刷新分组条：重新绑定滚动 + 测量列宽（layout 异步，nextTick 后再 rAF 补一次） */
  ...
  /** 按 el-table 渲染顺序重排：左固定 → 中间 → 右固定（与表头 th 的 DOM 顺序、measuredWidths 对齐） */
  const colsInRenderOrder = (cols: ColumnOption[]): ColumnOption[] => {
    const left: ColumnOption[] = []
    const center: ColumnOption[] = []
    const right: ColumnOption[] = []
    cols.forEach((c) => {
      if ((c as any).fixed === 'right') right.push(c)
      else if ((c as any).fixed === true || (c as any).fixed === 'left') left.push(c)
      else center.push(c)
    })
    return [...left, ...center, ...right]
  }

  /** 把连续同组列合并为分组段，未分组列合并为占位段；宽度取实测列宽之和 */
  const groupBarSegments = computed(() => {
    if (!groupBarEnabled.value) return []
    const cols = colsInRenderOrder(displayColumns.value)
    const widths = measuredWidths.value
    if (!cols.length || widths.length !== cols.length) return []
    const segs: Array<{ isGroup: boolean; width: number; title?: string; color?: string }> = []
    let i = 0
    while (i < cols.length) {
      const gkey = (cols[i] as any).group as string | undefined
      if (gkey && groupMetaMap.value.has(gkey)) {
        let width = 0
        while (i < cols.length && (cols[i] as any).group === gkey) {
          width += widths[i] || 0
          i++
        }
        const g = groupMetaMap.value.get(gkey)!
        segs.push({ isGroup: true, width, title: g.title, color: g.color || '#4f7cff' })
      } else {
        let width = 0
        while (i < cols.length) {
          const k = (cols[i] as any).group as string | undefined
          if (k && groupMetaMap.value.has(k)) break
          width += widths[i] || 0
          i++
        }
        segs.push({ isGroup: false, width })
      }
    }
    return segs
  })
  ...
  // 列变化/数据变化/分组变化后重新测量
  watch(
    [() => displayColumns.value, () => props.data, () => props.groups],
    () => refreshGroupBar(),
  ...
  const handleCurrentChange = (val: number) => {
    ...
    if (props.scrollPageToTopOnPageChange) {
      scrollToTop()
    } else {
      scrollTableToTop()
    }
  ...
  // 彩色分组条高度常量（标签 + 底部色线 + 间距）
  const GROUP_BAR_HEIGHT = 30
  ...
  onMounted(() => {
    nextTick(() => {
      observeTableHeader()
      updateContainerTopOffset()
      // 分组条：绑定滚动 + 测量列宽 + 监听表头/表体尺寸变化（列宽随 fit 变化时重新测量）
      if (groupBarEnabled.value) {
        refreshGroupBar()
        const tableRoot = getOwnTableRoot()
        const targets = [
          tableRoot?.querySelector<HTMLElement>(
            ':scope > .el-table__inner-wrapper > .el-table__header-wrapper table'
          ),
          tableRoot?.querySelector<HTMLElement>(
            ':scope > .el-table__inner-wrapper > .el-table__body-wrapper table'
          )
        ].filter((target): target is HTMLElement => !!target)
        if (targets.length) {
          groupResizeObserver = new ResizeObserver(() => measureColumns())
          targets.forEach((el) => groupResizeObserver!.observe(el))
        }
      }
    })
...
<style lang="scss" scoped>
  ...
  /* 开启分组条时，表格紧贴分组条下方（去掉默认的 10px 上间距） */
  .has-group-bar :deep(.el-table) {
    margin-top: 0;
  }

  /* 彩色分组条：位于表格上方（表格外），裁掉横向溢出，内层 track 随表体滚动做 translateX */
  .atg-group-bar {
    width: 100%;
    height: 30px;
    overflow: hidden;
    ...
    .atg-group-bar__track {
      display: flex;
      height: 100%;
      will-change: transform;
    }

    /* 每一段：分组段或占位段（未分组列，如日期列） */
    .atg-group-seg {
      position: relative;
...
<style lang="scss">
  /* ElTooltip 的 popper 挂到 body，须非 scoped */
  .art-table-ellipsis-tooltip.el-popper {
  ...
  .art-table-ellipsis-tooltip.el-popper .el-popper__inner {
    box-sizing: border-box;
    max-width: 100%;
    max-height: inherit;
    overflow-y: auto;
