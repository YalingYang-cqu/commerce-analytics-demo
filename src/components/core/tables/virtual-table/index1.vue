...
<script setup>
  ...
  const handleTableScroll = () => {
    Promise.resolve().then(() => measureRenderedRowHeights())
  }
...
<style scoped>
  ...
  /* 确保虚拟滚动容器有滚动条 */
  :deep(.virtual-vxe-table .vxe-table--body-wrapper) {
    overflow-y: auto !important;
    overflow-x: auto !important;
  }

  /* 自定义滚动条样式 */
  :deep(.virtual-vxe-table .vxe-table--body-wrapper::-webkit-scrollbar) {
    width: 8px;
    height: 8px;
  }

  :deep(.virtual-vxe-table .vxe-table--body-wrapper::-webkit-scrollbar-thumb) {
    background-color: rgba(0, 0, 0, 0.2);
    border-radius: 4px;
  }

  :deep(.virtual-vxe-table .vxe-table--body-wrapper::-webkit-scrollbar-thumb:hover) {
