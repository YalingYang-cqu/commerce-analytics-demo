  ...
  <div class="table-header" id="art-table-header">
    ...
    <div class="right">
      ...
      <MkTableFilterDrag
        ...
        :hasSetChild="false"
        :isSort="true"
        @showItemChange="handleShowItemChange"
...
<script lang="ts" setup>
  ...
  // 显式引入扁平版列设置弹窗：避免被全局自动注册解析成 group 版（group 版未分组列禁用拖拽）
  ...
  //   { immediate: true }
  // )
  const initialCols = columns.value?.filter((v: any) => !v.hide) || []
  ...
  watch(
    () => columns.value?.filter((v: any) => !v.hide) || [],
    (filtered) => {
