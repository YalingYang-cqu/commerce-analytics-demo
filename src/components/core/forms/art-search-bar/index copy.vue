          ...
          >
            <slot :name="item.key" :item="item" :modelValue="modelValue">
              <component
                ...
                @change="
                  item.trigger === 'change' || item.type === 'select' ? handleSearch() : undefined
                "
                @clear="handleSearch"
                size="small"
              >
                ...
                <!-- 分类 -->
                <!-- <tree-select :data="categoryApiArr"
                    width="160"
                    :defaultProps="categoryProps"
                    :checkedKeys="modelValue[item.key]"
                    @popoverHide="FnChooseCategory"/> -->
                <!-- 动态插槽支持 -->
...
<script setup lang="ts">
   ...
   * 可见的表单项
   */
  const visibleFormItems: any = computed(() => {
