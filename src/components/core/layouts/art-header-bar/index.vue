  ...
  <div class="layout-top-bar" :class="[tabStyle]">
    <div class="menu">
      ...
      <div class="right">
        ...
        <!-- 下载中心 -->
        <div class="btn-box down-box" @click="goPage('/download-center')">
          <div class="btn down-btn">
        ...
        <div class="user">
          ...
          >
            <template #reference>
              <div class="user-reference" role="button" tabindex="0">
                <img class="cover" :src="avatarSrc" alt="" />
                <span class="user-reference-name">{{ displayUserName }}</span>
                <i class="iconfont-sys user-reference-caret">&#xe6b8;</i>
              </div>
            </template>
            <template #default>
...
<script setup lang="ts">
  ...
  import defaultAvatar from '@/assets/svg/man.svg'
  ...
  const { language, info: userInfo } = storeToRefs(userStore)
  const { menuList } = storeToRefs(menuStore)

  const displayUserName = computed(() => userInfo.value.userName || userInfo.value.account || '-')
  const displayPhone = computed(() => userInfo.value.phoneNum || '-')
   ...
   * 打开 elementPlus 页面
   */
  // const toElementPlus = (): void => {
  //   window.open(WEB_LINKS.ELEMENT_PLUS)
  // }
