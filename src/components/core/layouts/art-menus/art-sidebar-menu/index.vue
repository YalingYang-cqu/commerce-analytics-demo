  ...
  >
    ...
    >
      <ElScrollbar style="height: calc(100% - 10px)">
        ...
        <div v-if="isDev" class="dev-seting">
          <DevSetting />
        </div>
      </ElScrollbar>
...
<script setup lang="ts">
  ...
  import DevSetting from '@/components/core/DevSetting/index.vue'
  ...
  const menuList = computed(() => {
    ...
    let allMenus = menuStore.menuList

    // 外部系统入口始终排在最后
    allMenus = [...allMenus]
    console.log(allMenus, ' allMenus allMenus allMenus')
    const externalIndex = allMenus.findIndex(
      (menu) => menu.path === '/external-system' || menu.name === 'ExternalSystem'
    )
    if (externalIndex !== -1) {
      const externalMenu = allMenus.splice(externalIndex, 1)[0]
      allMenus.push(externalMenu)
    }
