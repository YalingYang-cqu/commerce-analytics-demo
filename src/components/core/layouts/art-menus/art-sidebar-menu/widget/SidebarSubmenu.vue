...
<script setup lang="ts">
  ...
  /**
   * 过滤后的菜单项列表
   * 只显示未隐藏的菜单项，并确保外部系统入口在最后
   */
  const filteredMenuItems = computed(() => {
    const filtered = filterRoutes(props.list)
    const externalIndex = filtered.findIndex(
      (item) => item.path === '/external-system' || item.name === 'ExternalSystem'
    )
    if (externalIndex !== -1 && externalIndex !== filtered.length - 1) {
      const externalItem = filtered.splice(externalIndex, 1)[0]
      filtered.push(externalItem)
    }
    return filtered
  })
...
<script lang="ts">
  ...
  const MenuItemIcon = defineComponent({
    ...
    setup(props) {
      return () =>
        h('i', {
          class: 'menu-icon iconfont-custom',
